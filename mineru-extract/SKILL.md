...
4. When the user does NOT specify an output path (`-o`), the agent MUST generate a default output directory in the **current working directory** to prevent file overwrites. Use:

```
./mineru_extract/<name>_<hash>/
```
...
**Output location:** All extracted files are saved in the **current working directory** under `./mineru_extract/` by default. Users can override this by specifying `-o <path>`.
...
After `flash-extract` completes successfully:

1. **If user wants only the .md file**: Clean up all intermediate files in the output directory (images, thumbnails, temp files, etc.). Run cleanup:
   - On Linux/macOS: `find <output_dir> -type f ! -name "*.md" -delete && find <output_dir> -type d -empty -delete`
   - On Windows PowerShell: `Get-ChildItem -Path <output_dir> -Recurse -File | Where-Object { $_.Extension -ne ".md" } | Remove-Item -Force; Get-ChildItem -Path <output_dir> -Recurse -Directory | Where-Object { (Get-ChildItem $_.FullName -Force).Count -eq 0 } | Remove-Item -Force`
2. **Append a brief hint** (only if user has not seen it in this session):
...
**Rule**: If user explicitly says "只要 md 文件" or "只给我 md 文件", ALWAYS run cleanup after extraction. Do NOT skip this step.
...
|--------|----------|-----------------|
| `https://arxiv.org/pdf/2509.22186` | `2509.22186` | `./mineru_extract/2509.22186_a3f2b1/` |
| `https://arxiv.org/pdf/2509.200` | `2509.200` | `./mineru_extract/2509.200_c7e9d4/` |
| `C:\Users\test\report.pdf` | `report` | `./mineru_extract/report_d4e8f2/` |
