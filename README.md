...
本仓库提供 8 个围绕“做科研流程”的技能：材料构思 → 文献导读 →（社科基金题目评审/选题说明）→ 开题评审 → 同行评议（审稿与回复）→ Slides/PPT（Marp）+ 知识库问答（RAGflow）。
...
| `ragflow-client`（知识库问答）             | 需要向 RAGflow 知识库提问；希望回答带引用来源                             | ragflow、知识库问答、查知识库、带引用、reference                                                                                      | `用 ragflow 知识库回答，并给出引用来源：...`                                               |
...
### 2) 安装 CoPaw 技能（同行评议 + 社科基金写作 + RAGflow 知识库问答）
