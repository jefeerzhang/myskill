...
| `同行评议/`                     |     — | 同行评审/审稿意见回复：方法学、统计有效性与报告规范检查   | 论文全文、审稿意见、基金申请书 | 可选：图表/规范清单（CONSORT/STROBE/PRISMA 等） | 对话输出（可整理为审稿报告） |
...
| `peer-review`（同行评议）                  | 期刊审稿；基金评审；稿件修改；回复审稿意见（Response/Rebuttal）           | 同行评议、同行评审、审稿、审稿意见、response to reviewers、rebuttal、投稿、修改说明                                                    | `请对这篇论文进行同行评议：...` / `帮我逐条回复以下审稿意见：...`                         |
